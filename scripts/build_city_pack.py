    ...
    try:
        ...
        for el in data.get("elements", []):
            ...
            if not lat or not lon:
                continue
            tags = el.get("tags", {})
            ltype = (tags.get("leisure") or tags.get("tourism")
                     or tags.get("amenity") or tags.get("natural") or "landmark")
            results.append({
                "name": name,
                "type": ltype,
                "lat": float(lat),
...
INTER_CITY: list[dict] = [
    {"id": "sf-portland-coast-starlight",
     "from": "san_francisco", "to": "portland",
     "mode": "train", "operator": "Amtrak Coast Starlight",
     "duration_hours": 17.5,
    ...
    {"id": "sf-la-coast-starlight",
     "from": "san_francisco", "to": "los_angeles",
     "mode": "train", "operator": "Amtrak Coast Starlight",
     "duration_hours": 11.5,
     "departure_hub": "Emeryville (shuttle from SF BART/ferry)",
    ...
    {"id": "sf-la-flight",
     "from": "san_francisco", "to": "los_angeles",
     "mode": "flight", "operator": "various (SFO)",
     "duration_hours": 1.5,
     "departure_hub": "SFO (BART SFO station)", "arrival_hub": "LAX",
     "notes": "30+ daily flights. BART from downtown to SFO in ~30 min."},
    {"id": "sf-oakland-bart",
     "from": "san_francisco", "to": "oakland",
     "mode": "rail", "operator": "BART",
     "duration_hours": 0.25,
     "departure_hub": "Embarcadero/Civic Center BART", "arrival_hub": "12th St / 19th St Oakland",
     "notes": "Through the Transbay Tube. ~15 min, very frequent."},
    {"id": "sf-berkeley-bart",
     "from": "san_francisco", "to": "berkeley",
     "mode": "rail", "operator": "BART",
     "duration_hours": 0.4,
     "departure_hub": "Embarcadero BART", "arrival_hub": "Downtown Berkeley BART",
     "notes": "Red or yellow line, ~25 min."},
    {"id": "sf-sj-caltrain",
     "from": "san_francisco", "to": "san_jose",
     "mode": "rail", "operator": "Caltrain",
     "duration_hours": 1.25,
     "departure_hub": "4th & King (Caltrain station, Muni N/T)", "arrival_hub": "San José Diridon",
     "notes": "Peninsula commuter rail, bullet trains ~1h15, locals longer."},
    {"id": "sf-marin-ferry",
     "from": "san_francisco", "to": "marin_county",
     "mode": "ferry", "operator": "Golden Gate Ferry",
     "duration_hours": 0.5,
     "departure_hub": "Ferry Building", "arrival_hub": "Sausalito or Larkspur",
     "notes": "Scenic bay crossing with Golden Gate views. Sausalito = bike-friendly."},
    {"id": "sf-amsterdam",
     "from": "san_francisco", "to": "amsterdam",
     "mode": "flight", "operator": "KLM / United (SFO)",
     "duration_hours": 11.5,
     "departure_hub": "SFO International Terminal", "arrival_hub": "Amsterdam Schiphol",
     "notes": "Direct nonstop ~11h. Nine-hour time difference."},
]
...
def _assign_landmark_neighborhoods(
    landmarks: list[dict], neighborhoods: list[dict]
) -> list[dict]:
...
def _assign_transit_neighborhoods(
    stations: list[dict], neighborhoods: list[dict]
) -> list[dict]:
...
def _merge_osm_neighborhoods(
    curated: list[dict], osm: list[dict]
) -> list[dict]:
    ...
    for osm_n in osm:
        ...
        too_close = any(
            _haversine_km(osm_n["lat"], osm_n["lon"], lat, lon) < 0.5
...
def _merge_osm_transit(
    curated_bart: list[dict], curated_muni: list[dict],
    osm_bart: list[dict], osm_muni: list[dict],
    ...
    for osm_s in osm_bart:
        ...
        if key in bart_by_name:
            ...
            osm_dist = _haversine_km(
                existing["lat"], existing["lon"], osm_s["lat"], osm_s["lon"]
            )
...
def _build_transit_graph(
    bart: list[dict], muni: list[dict], neighborhoods: list[dict]
) -> dict:
...
def build_pack(output_dir: Path, offline: bool = False) -> None:
    ...
    all_bart, all_muni = _merge_osm_transit(
        BART_STATIONS[:], MUNI_METRO[:], osm_bart, osm_muni
    )
...
def main() -> None:
    ...
    if args.city != "san_francisco":
        print(f"Error: only 'san_francisco' is supported in this version", file=sys.stderr)
        sys.exit(1)
