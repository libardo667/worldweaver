...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "actor_type", sa.String(length=20), nullable=False, server_default="human"
        ),
        ...
        sa.Column(
            "origin", sa.String(length=20), nullable=False, server_default="migrated"
        ),
    ...
    op.create_index(
        op.f("ix_federation_actors_handle"),
        "federation_actors",
        ["handle"],
        unique=True,
    )
    op.create_index(
        op.f("ix_federation_actors_source_actor_id"),
        "federation_actors",
        ["source_actor_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_federation_actor_auth_email"),
        "federation_actor_auth",
        ["email"],
        unique=True,
    )
    op.create_index(
        op.f("ix_federation_actor_auth_username"),
        "federation_actor_auth",
        ["username"],
        unique=True,
    )
    ...
    with op.batch_alter_table("players") as batch_op:
        ...
        batch_op.create_index(
            batch_op.f("ix_players_actor_id"), ["actor_id"], unique=True
        )

    with op.batch_alter_table("session_vars") as batch_op:
        batch_op.add_column(sa.Column("actor_id", sa.String(length=36), nullable=True))
        batch_op.create_index(
            batch_op.f("ix_session_vars_actor_id"), ["actor_id"], unique=False
        )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_federation_actor_auth_username"), table_name="federation_actor_auth"
    )
    op.drop_index(
        op.f("ix_federation_actor_auth_email"), table_name="federation_actor_auth"
    )
    op.drop_table("federation_actor_auth")
    op.drop_index(
        op.f("ix_federation_actors_source_actor_id"), table_name="federation_actors"
    )
