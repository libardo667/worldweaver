...
const PARTICIPATION_MODE_KEY = "ww.client.participation_mode";
...
export function clearSessionStorage(): void {
  ...
  localStorage.removeItem(PARTICIPATION_MODE_KEY);
...
export type ParticipationMode = "observer" | "participant";

export function getParticipationMode(): ParticipationMode | null {
  const value = String(localStorage.getItem(PARTICIPATION_MODE_KEY) || "").trim();
  if (value === "observer" || value === "participant") {
    return value;
  }
  return null;
}
...
export function getSelectedShardUrl(): string | null {
  return localStorage.getItem(SELECTED_SHARD_URL_KEY);
}

export function setSelectedShardUrl(url: string): void {
  localStorage.setItem(SELECTED_SHARD_URL_KEY, url);
}

export function clearSelectedShardUrl(): void {
  localStorage.removeItem(SELECTED_SHARD_URL_KEY);
}
