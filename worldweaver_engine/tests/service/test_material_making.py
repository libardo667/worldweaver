...
from src.services.material_making import (
    initialize_material_pools,
    make_durable_object,
    making_catalog,
)
...
def _session(
    db,
    session_id: str = "maker-session",
    actor_id: str = "actor-maker",
    location: str = "Alderbank Workshop",
) -> None:
    db.add(
        SessionVars(
...
def test_catalog_is_elective_and_exact_location_scoped(db_session, game_rules):
    ...
    assert {item["material_id"] for item in catalog["materials"]} == {
        "reclaimed_clay",
        "fallen_wood",
    }
    assert {item["recipe_id"] for item in catalog["recipes"]} == {
        "small_clay_cup",
        "wooden_token",
    }
...
def test_making_consumes_material_and_creates_evidence_backed_object(
...
def test_making_retry_does_not_consume_twice(db_session, game_rules):
    ...
    wood = (
        db_session.query(MaterialPool)
        .filter(MaterialPool.material_id == "fallen_wood")
        .one()
    )
...
def test_materials_replenish_by_bounded_elapsed_intervals(db_session, game_rules):
    ...
    assert (
        clay.available_units == 2
    )  # two intervals added four units; the cup consumed two
    assert clay.last_replenished_at == now.replace(tzinfo=None)


def test_event_failure_rolls_back_materials_and_object(
