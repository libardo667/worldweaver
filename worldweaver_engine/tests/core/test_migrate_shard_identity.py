...
def test_migration_is_private_and_idempotent(tmp_path) -> None:
    ...
    assert "WW_NODE_PRIVATE_KEY_PATH=identity/node.key" in (shard / ".env").read_text(encoding="utf-8")
    ...
    assert (shard / "docker-compose.yml").read_text(encoding="utf-8").count("./identity:/app/identity:ro") == 1
    assert (shard / ".gitignore").read_text(encoding="utf-8").count("identity/node.key") == 1
...
def test_world_folder_uses_its_directory_name_when_legacy_env_has_no_id(tmp_path) -> None:
