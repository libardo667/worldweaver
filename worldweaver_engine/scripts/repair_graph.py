...
def repair_graph(db_url: str, k: int, max_km: float, dry_run: bool) -> dict:
    ...
    with Session() as session:
        ...
        all_loc_nodes = (
            session.query(WorldNode).filter(WorldNode.node_type == "location").all()
        )
        cp_locations = [
            n
            for n in all_loc_nodes
            if (n.metadata_json or {}).get("source") == "city_pack"
        ]

        # All city-pack landmark nodes (real SF places, not yet in path graph)
        all_lm_nodes = (
            session.query(WorldNode).filter(WorldNode.node_type == "landmark").all()
        )
        cp_landmarks = [
            n
            for n in all_lm_nodes
            if (n.metadata_json or {}).get("source") == "city_pack"
        ]
        ...
        loc_georef_valid = [
            t for t in loc_georef if t[1] is not None and t[2] is not None
        ]
        ...
        def _connect(node: "WorldNode", candidates_georef: list, label: str) -> None:
            ...
            if olat is not None and olon is not None and candidates_georef:
                ...
                for neighbor, dist_km in chosen:
                    ...
                    print(
                        f"    {label}: {node.name} ↔ {neighbor.name}  ({dist_km:.2f} km)"
                    )
            else:
                result["no_coords"] += 1
                alpha = sorted(
                    ...
                    key=lambda n: (
                        abs(ord(n.name[0].lower()) - ord(node.name[0].lower()))
                        if n.name and node.name
                        else 999
                    ),
                )[:k]
...
def main() -> int:
    ...
    try:
        counts = repair_graph(
            db_url, k=args.k, max_km=args.max_km, dry_run=args.dry_run
        )
    ...
    if not args.dry_run:
        print(
            ...
            + (
                f", {counts['no_coords']} had no coordinates"
                if counts["no_coords"]
                else ""
            )
        )
