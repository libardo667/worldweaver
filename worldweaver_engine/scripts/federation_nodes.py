...
def _node_payload(node: FederationShard) -> dict[str, object]:
    return {
        ...
        "key_recovered_at": (
            node.key_recovered_at.isoformat() if node.key_recovered_at else None
        ),
...
def _trust_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--shard-type", required=True, choices=("city", "world", "neighborhood")
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Manage nodes admitted to this federation directory."
    )
    commands = parser.add_subparsers(dest="command", required=True)

    commands.add_parser("list", help="list admitted and revoked nodes")
    history = commands.add_parser(
        "history", help="show the append-only trust history for one node"
    )
    ...
    recover = commands.add_parser(
        "recover", help="replace the key of a previously revoked node"
    )
...
def main() -> int:
    ...
    if settings.shard_type != "world":
        print(
            "ERROR: node trust can only be managed on a federation directory.",
            file=sys.stderr,
        )
        return 1
    ...
    with SessionLocal() as db:
        try:
            if args.command == "list":
                nodes = (
                    db.query(FederationShard).order_by(FederationShard.shard_id).all()
                )
                payload: object = [_node_payload(node) for node in nodes]
            elif args.command == "history":
                events = (
                    db.query(FederationNodeTrustEvent)
                    .filter(FederationNodeTrustEvent.node_id == args.node_id)
                ...
                payload = _node_payload(
                    revoke_node(db, node_id=args.node_id, reason=args.reason)
                )
            else:
