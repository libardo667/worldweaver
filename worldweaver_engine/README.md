...
World geography is seeded from city packs (`data/cities/<city>/`). A city pack contains neighborhoods,
transit, landmarks, street corridors, weather settings, and city-owned travel hubs. Building is
city-agnostic: add a config under `scripts/city_configs/`, then run `scripts/build_city_pack.py`. Real-place
configs can use OpenStreetMap enrichment or `--offline` for the curated baseline. An original fictional
config sets `fictional: true`; the builder then skips OpenStreetMap, labels every place as fictional, and
uses the source and license declared by the pack author. The builder runs the same structured validator
intended for the steward City Studio, so broken core IDs, coordinates, adjacency, hubs, and routes fail
before files are written. Seeding is a one-time founding operation; do not rebuild the ground under an
inhabited city.
