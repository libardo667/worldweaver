...
def _events_kept(memory_dir):
    return [
        ...
        if e.get("event_type") == "memory_kept"
    ]


def test_pulse_parses_keep_as_string_list_or_objects():
    assert [
        k.note for k in Pulse.from_dict({"keep": "the keeper moves to NL"}).keepsakes
    ] == ["the keeper moves to NL"]
    assert [k.note for k in Pulse.from_dict({"keep": ["a", "b"]}).keepsakes] == [
        "a",
        "b",
    ]
    assert [
        k.note
        for k in Pulse.from_dict(
            {"keep": [{"note": "x"}, {"text": "y"}, {"note": ""}]}
        ).keepsakes
    ] == ["x", "y"]
    ...
    pulse = Pulse.from_dict(
        {
            "felt_sense": "noted",
            "keep": [
                "the keeper is moving to the Netherlands",
                "I started a Threshold Log",
            ],
        }
    )
    summary = route_pulse(tmp_path, pulse, now="2026-06-03T00:00:00+00:00")
    assert summary["memories_kept"] == 2
    assert {e["payload"]["note"] for e in _events_kept(tmp_path)} == {
        "the keeper is moving to the Netherlands",
        "I started a Threshold Log",
    }
...
def test_derive_memories_newest_first_and_dedupes_on_rekeep(tmp_path):
    for i, note in enumerate(["one", "two", "one"]):  # 'one' kept again, latest
        route_pulse(
            tmp_path,
            Pulse.from_dict({"keep": [note]}),
            now=f"2026-06-03T00:0{i}:00+00:00",
        )
...
def test_derive_memories_caps_at_limit(tmp_path):
    for i in range(20):
        route_pulse(
            tmp_path,
            Pulse.from_dict({"keep": [f"note-{i:02d}"]}),
            now=f"2026-06-03T00:{i:02d}:00+00:00",
        )
...
def test_kept_memory_survives_ledger_eviction(tmp_path):
    ...
    route_pulse(
        tmp_path,
        Pulse.from_dict({"keep": ["the keeper moves to NL in autumn"]}),
        now="2026-06-03T00:00:00+00:00",
    )
    assert (tmp_path / KEPT_STORE_NAME).exists()
    (tmp_path / "runtime_ledger.jsonl").write_text(
        "", encoding="utf-8"
    )  # simulate full eviction
    assert [m["note"] for m in memories(tmp_path)] == [
        "the keeper moves to NL in autumn"
    ]
...
def test_memories_rescues_a_ledger_only_keepsake_into_the_durable_store(tmp_path):
    ...
    append_runtime_event(
        ...
        payload={
            "note": "an old morning thought",
            "kept_ts": "2026-06-03T08:00:00+00:00",
        },
    )
    assert not (tmp_path / KEPT_STORE_NAME).exists()
    assert "an old morning thought" in [
        m["note"] for m in memories(tmp_path)
    ]  # read rescues it
    assert (tmp_path / KEPT_STORE_NAME).exists()
    (tmp_path / "runtime_ledger.jsonl").write_text("", encoding="utf-8")
    assert "an old morning thought" in [
        m["note"] for m in memories(tmp_path)
    ]  # still here after eviction
...
def test_memory_recall_ranks_by_relevance_not_recency():
    ...
    hits = asyncio.run(
        mr.recall(notes, "tell me about the Netherlands windmills and the keeper")
    )
    assert hits and hits[0]["note"] == notes[0]  # relevance wins, not the most recent
    assert notes[1] not in [
        h["note"] for h in hits[:1]
    ]  # the unrelated printer note doesn't lead
...
def test_memory_recall_caches_embeddings_per_note():
    ...
    asyncio.run(
        mr.recall(
            ["the rain is thin today", "the dust drifts here"], "rain on the glass"
        )
    )
...
def test_memory_recall_diversity_avoids_near_dup_pileup():
    ...
    div = [
        ...
        for h in asyncio.run(
            MemoryRecall(DeterministicEmbedder()).recall(
                notes, moment, top_k=2, diversity=0.85
            )
        )
    ]
    rel = [
        ...
        for h in asyncio.run(
            MemoryRecall(DeterministicEmbedder()).recall(
                notes, moment, top_k=2, diversity=0.0
            )
        )
    ]
    assert distinct in div  # diversity surfaces the genuinely different recollection
    assert distinct not in rel and all(
        n in near for n in rel
    )  # pure relevance piles up near-dups
...
def test_memory_novel_drops_restatements_keeps_distinct():
    ...
    cands = [
        "the keeper is moving to the netherlands very soon indeed",
        "the printer makes a clicking sound at night",
    ]
...
def test_memory_novel_drops_dups_within_the_batch():
    mr = MemoryRecall(DeterministicEmbedder())
    cands = [
        "the groove is worn so stop polishing the same shape",
        "the groove is worn so quit polishing that same shape",
    ]
