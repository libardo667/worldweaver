...
from src.runtime.prediction import (
    derive_anchor_scores,
    derive_prediction_scores,
    summarize_anchor_prediction,
...
def _surprise(ts, features):
    ...
    return {
        ...
        "payload": {
            "observed_ts": ts,
            "magnitude": max(d for _, _, d in features),
            "features": [{"scope": s, "tag": t, "delta": d} for s, t, d in features],
        },
    }
...
def test_summary_triad_distinguishes_dark_room_from_learning():
    ...
    events = [
        _cast(
            "silent", "2026-06-03T00:00:00+00:00", {}, half_life=120.0
        ),  # claims nothing → dropped (no features)
        _cast("p1", "2026-06-03T00:05:00+00:00", {"rain": 0.6}),
        _surprise(
            "2026-06-03T00:06:00+00:00", [("self", "rain", 0.05)]
        ),  # tiny → clean-ish
    ]
...
def test_weighted_miss_counts_a_mattering_feature_for_more():
    ...
    assert (
        c["weighted_miss"] > f["weighted_miss"]
    )  # being wrong about what matters costs more
...
def test_claim_mattering_flags_the_dull_world_predictor():
    ...
    events = [
        ...
        _surprise(
            "2026-06-03T00:01:00+00:00", [("self", "dust", 0.02)]
        ),  # below floor → clean
    ]
    ...
    assert (
        s["claim_mattering"] < 0.1
    )  # but it predicted nothing it cares about — the dull-world tell
...
def _anchor_obs(ts, field):  # field: {anchor: salience}
    return {
        ...
        "payload": {
            "observed_ts": ts,
            "anchors": [{"anchor": k, "salience": v} for k, v in field.items()],
        },
    }
...
def test_anchor_prediction_hits_when_the_anchor_stays_salient():
    events = [
        _cast("p1", "2026-06-03T00:00:00+00:00", {"the keeper": 0.6}, scope="anchors"),
        _anchor_obs("2026-06-03T00:01:00+00:00", {"the keeper": 0.7}),
    ]
...
def test_anchor_prediction_misses_when_anchor_absent():
    events = [
        _cast("p1", "2026-06-03T00:00:00+00:00", {"the keeper": 0.8}, scope="anchors"),
        _anchor_obs("2026-06-03T00:01:00+00:00", {"the hearth": 0.7}),
    ]
    [s] = derive_anchor_scores(events)
...
def test_anchor_claim_mattering_uses_soul_weights():
    events = [
        _cast(
            ...
            {"the keeper": 0.6, "the dust": 0.6},
            scope="anchors",
        ),
        _anchor_obs("2026-06-03T00:01:00+00:00", {"the keeper": 0.6}),
    ]
    s = summarize_anchor_prediction(
        events, weights={"the keeper": 0.9, "the dust": 0.1}
    )
    assert (
        abs(s["mean_claim_mattering"] - 0.5) < 1e-6
    )  # (0.9 + 0.1) / 2 — and it actually varies, unlike the flat drives
...
def test_anchor_afterimage_does_not_drive_arousal_quiet_guarantee(tmp_path):
    ...
    assert (
        trace is None
    )  # the anchor lane is held out of the rhythm — no phantom surprise, no arousal
...
def test_anchor_gating_fires_on_appearance_not_disappearance(tmp_path):
    ...
    trace = observe_surprise(
        tmp_path,
        stimulus={"anchors": {"a sudden noise": 0.9}},
        now="2026-06-03T00:01:00+00:00",
        include_anchor_scope=True,
    )
