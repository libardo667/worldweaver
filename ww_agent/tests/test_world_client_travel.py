...
def test_world_client_exposes_the_recoverable_travel_contract():
    ...
    asyncio.run(client.get_pending_space_access_requests("resident-1", "Wayfarer Back Room"))
    asyncio.run(client.request_space_access("resident-1", "Wayfarer Back Room", "request-1"))
    asyncio.run(client.resolve_space_access_request("resident-1", "access-1", "admitted", "admit-1"))
    asyncio.run(client.set_space_access_mode("resident-1", "Wayfarer Back Room", "private", "mode-1"))
    asyncio.run(client.invite_to_space("resident-1", "resident-2", "Wayfarer Back Room", "invite-1"))
    asyncio.run(client.revoke_space_access("resident-1", "resident-2", "Wayfarer Back Room", "revoke-1"))
    ...
    assert client.calls == [
        ...
        (
            ...
            {"session_id": "resident-1", "location": "Wayfarer Back Room", "idempotency_key": "request-1", "note": ""},
        ),
        (
            "POST",
            "/api/world/access/requests/access-1/resolve",
            {"session_id": "resident-1", "decision": "admitted", "idempotency_key": "admit-1"},
        ),
        (
            "POST",
            "/api/world/access/mode",
            {"session_id": "resident-1", "location": "Wayfarer Back Room", "mode": "private", "idempotency_key": "mode-1"},
        ),
        (
            "POST",
            "/api/world/access/invite",
            {"session_id": "resident-1", "recipient_session_id": "resident-2", "location": "Wayfarer Back Room", "idempotency_key": "invite-1"},
        ),
        (
            "POST",
            "/api/world/access/revoke",
            {"session_id": "resident-1", "recipient_session_id": "resident-2", "location": "Wayfarer Back Room", "idempotency_key": "revoke-1"},
        ),
        (
            "POST",
