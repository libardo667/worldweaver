...
def test_pulls_article_headed_concrete_phrases():
    texts = [
        "The keeper's voice is a warmth against the cooling room.",
        "the tea in my cup is a dark eye.",
    ]
...
def test_cuts_at_internal_verbs_and_prepositions():
    ...
    got = {
        a["anchor"] for a in extract_anchors(["the house has settled into its bones"])
    }
    assert "house" in got
...
def test_filters_abstract_function_nouns():
    got = {
        a["anchor"]
        for a in extract_anchors(
            ["the moment passed", "a kind of feeling", "the way of things"]
        )
    }
    assert (
        got == set()
    )  # moment / feeling / way / things are all abstract → nothing concrete
...
def test_structured_entities_count_double_and_get_normalized():
    ...
    anchors = {
        a["anchor"]: a["salience"] for a in extract_anchors([], structured=["Sun_Li"])
    }
