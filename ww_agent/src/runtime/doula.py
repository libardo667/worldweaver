...
class DoulaLoop:
    ...
    async def _seed_and_spawn(
        ...
        *,
        ...
        # Persist only controls that still have an owner. CognitiveCore has no
        # independent wander/rest scheduler, so advertising those old loop-bank
        # controls here would promise behavior the resident host cannot provide.
        # home_location remains operational input for canon_reset after the
        # one-time entry token is consumed on first boot.
        default_tuning: dict = {
