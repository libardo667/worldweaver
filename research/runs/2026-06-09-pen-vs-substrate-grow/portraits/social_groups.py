...
def main():
    ...
    for loc, mem in clusters.items():
        ...
        for a, b in recip:
            print(f"       {R[a]['name'].split()[0]:8}<->{R[b]['name'].split()[0]:8} ({W(a,b)}/{W(b,a)})" + ("  *STRONG*" if (a, b) in strong else ""))
    ...
    for a, b in strong_cross:
        print(f"       {R[a]['name'].split()[0]:8}({R[a]['home'][:10]}) <-> {R[b]['name'].split()[0]:8}({R[b]['home'][:10]})  ({W(a,b)}/{W(b,a)})")
