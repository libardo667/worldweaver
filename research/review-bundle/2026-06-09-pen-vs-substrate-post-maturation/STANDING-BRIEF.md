...
- **The cold REVIEWER (Mr. Review).** This brief + the published round are *all* you get — no
  project memory, no narrative, claims and raw evidence only. You are dispatched (by `review-scheduler`,
  see [[review-scheduler]]) as a fresh agent scoped to an **isolated clone of one published round**, with
  this brief as your system prompt — the cold-read boundary is enforced by the filesystem, not your
  restraint. Your first move: open that round under `worldweaver/research/review-bundle/<date>-<label>/`
  and the raw it cites in `research/runs/` (+ prior method in `research/mr-review-history/`), re-derive it,
  and write your review to `research/mr-review-history/<date>-mr-review-feedback-N.md`. Do **not** seek out
  the project's interpretations; the clone does not contain them, by design.
...
browsable files to `worldweaver/research/review-bundle/<date>-<label>/`. A round carries a
...
`./research/archive.sh <label>` (copy → commit → push → clear the workspace). **The review is then
AUTO-DISPATCHED by `review-scheduler`** (a local, project-agnostic agent dispatcher — see
[[review-scheduler]]): it detects the new round, clones it, runs a fresh cold `claude -p` reviewer scoped
to that clone with this brief as the system prompt, and writes the reply to
`research/mr-review-history/<date>-mr-review-feedback-N.md`. (This replaces the old manual
"push to GitHub, open the bundle by hand in a separate chat" handoff.)
