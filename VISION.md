...
WorldWeaver's first-class product surface is a purpose-built web client that makes two kinds of exploration feel natural:

1. **Physical exploration** (moving through geography).
2. **Narrative exploration** (moving through meaning-space via choices and freeform actions).

The UI should feel like a calm instrument panel: the story is the main window, but the player always has a few tactile controls and a visible memory of what has become true.

### Core screen layout

- **Now panel (center)**: the current scene text plus 2–6 choice buttons.
- **Freeform input (always visible)**: a single input line for “Try anything…” actions.
- **What changed (collapsible strip)**: a small, human-readable summary of state changes from the last step (e.g., “Trust with Mira: up”, “Rumor: started”, “Bridge: damaged”).
- **Place panel (right)**:
  - current location card (name + one-line vibe),
  - time/weather badges,
  - a compact **3×3 compass** for movement,
  - nearby points-of-interest as chips.
- **Memory panel (left)**:
  - recent events (timeline),
  - notable people/factions encountered,
  - a search box for semantic “ask the world” queries (powered by world facts search).

### Exploration modes

The client supports three modes that are one click apart:

1. **Explore** (default)
   - Read scene, choose or type an action, move via compass.
   - The “What changed” strip provides immediate consequence legibility.
2. **Reflect** (chronicle/legends mode)
   - A timeline of events with “Because of…” context explaining a few key causal/thematic links.
   - Designed to be screenshot-friendly and to support DF-style retellability.
3. **Create** (player-facing steering, not authoring)
   - Lightweight preference controls (tone, safety boundaries, “surprise me (safe)”).
   - These are expressed as session variables and narrative-beat lens weights.

### Optional semantic constellation view

An optional visualization mode shows a semantic constellation of nearby narrative possibilities:

- nodes: storylets, people, places, themes,
- “you are here” point representing current context,
- nearby nodes pulsing as likely next events (but distant nodes still possible).

This is not required for play, but it is valuable for:
- debugging and tuning,
- “the world is alive” moments,
- shareable artifacts.
...
WorldWeaver should produce artifacts that make players want to tell stories about their runs:

- **Run chronicle export** (markdown + JSON): a readable legend plus structured facts.
- **Pinboard**: player-pinned events, people, and unresolved threads.
- **Seed + replay hooks**: optional run seed capture for “same start, different lives” experiments.

Artifacts should emphasize: what happened, what changed, and why it mattered.
