...
**V4 is operational. Federation architecture is active work.**

- SF city pack live (875 nodes, BART/Muni, landmarks); Portland seeded (45 nodes, Amtrak routes)
- `ww_agent` residents running continuously (M3 complete)
- DB-backed DM system live (replaced filesystem letters 2026-03-14)
- Nearby landmark travel with confirm/preview step shipped (M3.5 partial)
- `world_bootstrap_service` and `session/start` endpoint deleted (pruned 2026-03-14)
- Unified turn pipeline operational; storylet system demoted to fallback
- Active: M3.5 social awareness, inter-city travel mechanic, federation layer design

## Key Workspace Note

**`../ww_agent`** is a companion repo one level above this one (i.e. `../ww_agent` from the
project root). It is not in this workspace but is directly accessible. Agent behavior, DM
endpoints, travel state, and doula loop work live there. Any task touching agent loops,
inter-city travel, the DM system, grounding prompts, or SOUL files will require coordinated
changes in both repos. Always check `../ww_agent/src/` when a task description mentions agents.
...
| `SpatialNavigator` | **Pruned** ✅ | — |
| `world_bootstrap_service` + `session/start` | **Pruned** ✅ | — |
...
- [x] Nearby landmark travel with confirm/preview step (landmark nodes excluded from entry screen;
      client-side orphan escape with confirm step; landmark clicks through preview like all travel)
...
See `improvements/VISION.md` for full design rationale. V5 extends the shared world into a
network of city shards coordinated by a thin federation layer.

### Shard Architecture

Each city is a **self-contained shard**: own DB, own agent processes, own local facts.
Above that sits a **federation layer** holding inter-city truths: cross-city DMs,
traveler records, shared world events.

Concrete target layout:
```
ww_sf/    — SF shard (current V4 instance, 875 nodes)
ww_pdx/   — Portland shard (45 nodes, city pack already seeded, ready to activate)
ww_world/ — federation layer: cross-city DMs, traveler records, shared event stream
```

Portland city pack seeds safely today: `_upsert_world_node` / `_upsert_world_edge` are
additive upserts. Command: `seed_world.py --city-pack --city-id portland --no-reset --no-residents`

### Travel in the Federation

Both humans and agents travel between cities via city context switches — the mechanic is
symmetric. For agents, the `traveling` dormant state manages what their action loops do
during transit (no actions, ground loop fires on departure/arrival). This is an
implementation concern, not a design hierarchy.
...
When a shard goes offline, its agents go quiet. The world notices. Other agents react.
When the shard returns, its characters re-enter and catch up. Uptime is continuity;
downtime is a story beat.

### Architecture

- **Shard boundary**: each city stack is authoritative only for its own facts and agents
- **Federation contract**: cross-city DMs, traveler records, and shared world events live
  in `ww_world/` and are read-only from each shard's perspective
- **Conflict resolution**: first-commit-wins within a shard; federation layer is append-only
- **Observatory**: public read-only portal — event feed, character timelines, live world
  state. No login required.

### V5 Milestones

#### M0: Inter-City Travel (Active — prerequisite for shards)

Make Portland reachable and activate the embodiment mechanic for agents.
See `improvements/majors/07-inter-city-travel.md` for full spec.

- [ ] Fix digest + map endpoints to filter nodes by `session_vars.city_id` (not hardcoded SF)
- [ ] Human travel: instant city-switch via transit hub UI; map re-renders destination city
- [ ] Agent travel: `traveling` dormant state with `{destination_city, route_id, arrive_ts}`;
      fast loop skips action; ground loop fires departure + arrival grounding moments
- [ ] `GET /api/world/inter-city-routes?location={hub}` and `POST /api/game/travel/inter-city`
...
Hardware + software bundle. The city-pack seed is deterministic node infrastructure — it
runs once per node, ever, and should succeed without a high-capability founding pass.
Optional enrichment can be layered on later. Continuous narration (fast/slow loops,
grounding, doula soul seeding) runs on cheaper, faster models.
