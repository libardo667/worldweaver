...
- â€œsurprise me (safe)â€ and other diversity operators are not player-addressable.
...
2. Create mode UI includes:
   ...
   - â€œSurprise me (safe)â€ button (sends a standardized freeform action to `/api/action` plus sets a `surprise_safe=true` var).
...
## Acceptance Criteria
- [x] Create mode is reachable from the client and does not reset the session.
- [x] Preferences persist across reloads (stored locally and mirrored into session vars sent to API).
- [x] â€œSurprise me (safe)â€ triggers a freeform action and renders results in Explore mode.
- [x] No backend route or payload changes are required for the client-only version.
- [x] Existing backend tests remain green (`pytest -q`).

## Risks & Rollback
Primary risks:
- Player expects stronger guarantees than preference steering can provide (make UI copy clear: â€œsteers, doesnâ€™t dictateâ€).
