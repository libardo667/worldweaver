...
4. `frontend_source`: `completed` (slices 1-16 complete: moved App runtime/config helper block into `appHelpers`, extracted topbar/mode shell into `AppTopbar`, extracted Explore center-column composition into `ExploreCenterColumn`, extracted Explore routing into `ExploreMode`, unified replacement-session reset state wiring, centralized turn lifecycle begin/end helpers, extracted turn orchestration into `useTurnOrchestration` with world/scene/player narrator lane adapters, split Explore center lane rendering into explicit `SceneLanePanel` + `PlayerHintPanel`, centralized topbar runtime status derivation with feature-flagged lane/budget chips, formalized typed v3 metadata parsing/propagation (`projection_ref`, `clarity_level`, `lane_source`), extracted onboarding/reset/dev-reset lifecycle handlers into `useSessionLifecycle` with explicit cache invalidation seams, replaced App mode branch conditionals with typed `ModeRouter` payload routing, added projection-scoped prefetch status/budget cache seams in `sessionStore` + `usePrefetchFrontier` with metadata propagation callbacks, extracted mode-payload assembly into `useModeRouterPayload` with explicit onboarding/memory/lane group schemas, and introduced normalized shared `ExploreModePayload` lane contracts decoupled from view-internal flat prop shape; strict gate passes)
- Evidence:
...
28. `BATCH_B_FRONTEND_SOURCE_SLICE_15.md`
29. `BATCH_B_FRONTEND_SOURCE_SLICE_16.md`
