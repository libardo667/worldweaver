...
4. `frontend_source`: `in_progress` (slices 1-10 complete: moved App runtime/config helper block into `appHelpers`, extracted topbar/mode shell into `AppTopbar`, extracted Explore center-column composition into `ExploreCenterColumn`, extracted Explore routing into `ExploreMode`, unified replacement-session reset state wiring, centralized turn lifecycle begin/end helpers, extracted turn orchestration into `useTurnOrchestration` with world/scene/player narrator lane adapters, split Explore center lane rendering into explicit `SceneLanePanel` + `PlayerHintPanel`, and centralized topbar runtime status derivation with feature-flagged lane/budget chips; latest strict gate passes)
- Evidence:
