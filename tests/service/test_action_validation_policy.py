...
def test_validation_blocks_dropping_item_not_in_inventory():
    ...
    intent = StagedActionIntent(
        ...
        result=ActionResult(
            narrative_text="Text",
            state_deltas={"apple": "dropped", "gold": 5},
            reasoning_metadata={"rejected_keys": [], "validation_warnings": []}
        )
    )
    ...
    # Player has no apple
    state_manager.get_state_summary.return_value = {
        "inventory": {"items": {"apple": False, "sword": True}},
        "variables": {"location": "test"}
    }
...
def test_validation_implausible_if_all_deltas_blocked():
    ...
    intent = StagedActionIntent(
        ack_line="Okay.",
        result=ActionResult(
            narrative_text="You drop the missing item.",
            state_deltas={"apple": "dropped"},
            reasoning_metadata={"rejected_keys": [], "validation_warnings": []}
        )
    )
