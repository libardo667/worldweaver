...
- [x] Normal runtime readers use the checkpoint/current-state API and do not repeatedly parse the complete
      cold ledger.
...
### 2026-07-20 — production runtime leaves compatibility memory behind

The resident host now constructs the small reference loop, which does not load kept-memory side files or parse
the cold ledger during an ordinary poll or activation. The remaining complete-history callers are recovery,
migration, explicit historical scoring, or code retained with the non-production old core. This closes the
normal-reader acceptance criterion without migrating an obsolete duplicate memory authority into the new path.
