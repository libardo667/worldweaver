...
`prune/harness/` contains 11 markdown docs and 9 templates that
...
2. Extract the generic methodology into a clean, standalone
   `prune/harness/METHODOLOGY.md` pitched to an outside reader.
3. Update the 9 templates to remove hardcoded WorldWeaver paths; use
   `<project-name>` / `<repo-root>` placeholders instead.
4. Write a `prune/harness/PUBLICATION_NOTES.md` capturing what worked
...
- `prune/harness/README.md` — rewrite as external-facing pitch
- `prune/harness/METHODOLOGY.md` — create (core methodology doc)
- `prune/harness/PUBLICATION_NOTES.md` — create (retrospective)
- `prune/harness/templates/*.md` (9 files) — de-WorldWeaver-ify
...
- [ ] No doc in `prune/harness/` references WorldWeaver internals
